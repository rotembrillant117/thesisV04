...
case "$JAMO_TYPE" in
    en_ro_baseline)
        MAIN_DATASET_DIR="orig/en_ro_baseline"
        HOMOGRAPH_DATASET_DIR="orig/en_ro_baseline_homograph"
        FLORES_DATASET_DIR="orig/en_ro_flores"
        FLORES_HOMOGRAPH_DATASET_DIR="orig/en_ro_flores_homograph"
        POST_PROCESS="sentencepiece"
        ;;
    en_ro_cue)
        MAIN_DATASET_DIR="orig/en_ro_cue"
        HOMOGRAPH_DATASET_DIR="orig/en_ro_cue_homograph"
        FLORES_DATASET_DIR="orig/en_ro_flores_cue"
        FLORES_HOMOGRAPH_DATASET_DIR="orig/en_ro_flores_cue_homograph"
        POST_PROCESS="sentencepiece_cues"
        ;;
    *)
        echo "Unsupported --jamo-type: $JAMO_TYPE"
        echo "Expected one of: en_ro_baseline, en_ro_cue"
        exit 1
        ;;
esac
...
CUDA_VISIBLE_DEVICES=$DEVICE fairseq-train "examples/homograph_translation/data-bin/$EXPERIMENT_NAME" \
    ...
    --eval-bleu-detok-args '{"target_lang": "ro"}' \
...
cat gen.out.sys | sacremoses -l ro detokenize > gen.out.sys.detok
cat gen.out.ref | sacremoses -l ro detokenize > gen.out.ref.detok
...
cat gen.out.homograph.sys | sacremoses -l ro detokenize > gen.out.homograph.sys.detok
cat gen.out.homograph.ref | sacremoses -l ro detokenize > gen.out.homograph.ref.detok
...
cat gen.out.flores.sys | sacremoses -l ro detokenize > gen.out.flores.sys.detok
cat gen.out.flores.ref | sacremoses -l ro detokenize > gen.out.flores.ref.detok
...
cat gen.out.flores.homograph.sys | sacremoses -l ro detokenize > gen.out.flores.homograph.sys.detok
cat gen.out.flores.homograph.ref | sacremoses -l ro detokenize > gen.out.flores.homograph.ref.detok
